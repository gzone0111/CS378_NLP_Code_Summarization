...
class Beam(object):
    ...
    def topk_sampling(self, logits, k=1, temperature=1.0):
        logits /= temperature
        probs = torch.softmax(logits.flatten(), dim=-1)
        sampled_indices = torch.multinomial(probs, num_samples=k)
        values, indices = torch.topk(logits.flatten()[sampled_indices], k=k)
        sampled_indices = sampled_indices.view(*logits.shape[:-1], k)
    ...
    def advance(self, word_probs, attn_out):
        ...
        * `attn_out`- attention at the last step
        ...
        # best_scores, best_scores_id = self.topk_sampling(flat_beam_scores,k=self.size,temperature=1)
