...
def validate_official(args, data_loader, model, global_stats, mode='dev'):
    ...
    result['f1'] = f1
    filename = 'metrics_record.txt'
    with open(filename, 'a') as f:
        f.write(f'bleu:{bleu},rouge_l:{rouge_l},meteor:{meteor},precision:{precision},recall:{recall},f1:{f1}')
    if mode == 'test':
        ...
        logger.info('train valid official: '
                    'bleu = %.2f | rouge_l = %.2f | meteor = %.2f | ' %
                    (bleu, rouge_l, meteor) +
                    'Precision = %.2f | Recall = %.2f | F1 = %.2f | '
                    'examples = %d | ' %
                    (precision, recall, f1, examples) +
                    'train eval time = %.2f (s)' % eval_time.time())
    else:
...
def main(args):
    ...
    # TRAIN/VALID LOOP
        ...
        for epoch in range(start_epoch, args.num_epochs + 1):
            ...
            # train_result = validate_official(args,train_loader,model,stats,mode='train')
